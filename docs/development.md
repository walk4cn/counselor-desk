...
上述三个集合已经纳入浏览器 IndexedDB、Electron、单文件离线包、备份恢复和手机工作包的统一集合清单。它们属于 v4.4.5 候选；正式公开状态以 v4.4.5 Release 和 Pages 的真实结果为准。
...
最终候选 HTML 的 SHA-256 只在新版本发布产物生成后记录；最终扫描源码、文档、备份和发布产物不得发现常见真实 API key、GitHub token 或 Slack token 模式。该扫描不替代账户持有人撤销此前已暴露密钥。

v4.4.5 发布前仍必须由账户持有人撤销此前暴露过的密钥并换发新密钥，然后在具备 GitHub 推送和 Release 权限的环境中提交、推送、构建和部署；新密钥不得进入源码、文档、日志、备份或 Git。
...
发布顺序为：测试 → Windows → macOS → 网页产物 → Draft Release → 人工确认 → Pages。v4.4.4 已按此顺序完成历史发布；v4.4.5 任一步失败、跳过或缺少真实产物，都不能提升公开版本。
